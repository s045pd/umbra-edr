...
#
# Output:
#   images/screenshots/login.png   (relative to repo root)
#
set -euo pipefail
...
if ! "${SCRIPT_DIR}/node_modules/.bin/playwright" install chromium 2>&1 | grep -q "already installed" 2>/dev/null; then
...
if [ "${LITE_MODE}" = "false" ]; then
  if ! command -v docker &>/dev/null; then
    _warn "docker not found — falling back to --lite mode automatically"
    LITE_MODE=true
  fi
fi
...
if [ "${LITE_MODE}" = "true" ]; then
  ...
  if [ ! -d "${GUI_NEXT}/node_modules" ]; then
    ...
    npm --prefix "${GUI_NEXT}" install
  fi

  npm --prefix "${GUI_NEXT}" run build
...
EXPECTED="${REPO_ROOT}/images/screenshots/login.png"
if [ ! -f "${EXPECTED}" ]; then
  _err "login.png not found at ${EXPECTED}"
  exit 1
fi
SIZE=$(wc -c < "${EXPECTED}")
if [ "${SIZE}" -lt 5120 ]; then
  _warn "login.png is suspiciously small (${SIZE} bytes) — may be a blank page"
else
  _ok "login.png exists (${SIZE} bytes)"
fi
